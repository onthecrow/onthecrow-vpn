...
The desktop launcher for DeltaVPN. It runs the **shared Compose Multiplatform UI** (`composeApp`)
...
xray JSON (on desktop via the bundled `delta-convert`, see `PlatformXrayEngine.jvm.kt`), the tun
...
- **macOS VPN** — the production build is one notarized `DeltaVPN.app` with the system extension +
...
> extension + bridge into the jpackage `DeltaVPN.app`, deep-signs Developer ID, notarizes and
